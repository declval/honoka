...
const std::chrono::duration<long long> DAY = 24h;
...
const std::string PROGRAM = "honoka";
...
public:
  Application() : db_(nullptr) {
    ...
    const std::string query =
        "CREATE TABLE IF NOT EXISTS cards (front TEXT PRIMARY KEY, back TEXT "
        "NOT NULL, interval INTEGER NOT NULL DEFAULT 0, created_at TEXT "
        "DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP)";
  ...
  void add(const std::string &front, const std::string &back) {
    const std::string query = "INSERT INTO cards (front, back) VALUES (?, ?)";
  ...
  void list() {
    const std::string query =
        "SELECT front, back, interval, unixepoch(updated_at) FROM cards";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
      ...
      const std::string back = (const char *)sqlite3_column_text(stmt, 1);
      int interval = sqlite3_column_int(stmt, 2);
      long long updated_at = sqlite3_column_int64(stmt, 3);
  ...
  void next() {
    ...
    query = "UPDATE cards SET interval = ?, updated_at = CURRENT_TIMESTAMP "
            "WHERE front = ?";
  ...
  void remove(const std::string &front) {
    const std::string query = "DELETE FROM cards WHERE front = ?";
...
private:
  bool needs_review(int interval, long long updated_at) {
